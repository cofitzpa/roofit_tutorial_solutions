...
USER root
RUN apt-get -y --force-yes install libx11-dev libxpm-dev libxft-dev libxext-dev libpng3 libjpeg8 gfortran libssl-dev libpcre3-dev libgl1-mesa-dev libglew1.5-dev libftgl-dev libmysqlclient-dev libfftw3-dev libcfitsio3-dev graphviz-dev libavahi-compat-libdnssd-dev libldap2-dev libxml2-dev libafterimage0 libafterimage-dev
...
RUN /bin/bash -c "git clone --depth 1 http://root.cern.ch/git/root.git -b v5-34-21 --single-branch \
   ...
   && cd root \
   && ./configure --prefix=/usr/local --minimal --enable-x11 --enable-builtin-afterimage --enable-asimage \
           --enable-astiff --enable-builtin_ftgl --enable-builtin_glew --enable-builtin_pcre --enable-builtin-lzma \
